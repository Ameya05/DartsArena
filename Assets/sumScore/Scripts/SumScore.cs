...
public class SumScore {
	...
	public static int DartScore { get; set; }
    ...
    public static void Add (int pointsToAdd) {
        ...
        if (MgrSet()) {
			...
			mgr.UpdatedDartCount();
        }
    }

	public static void IncrementDartCount () {
		DartScore++; // Add points to current score
	}
	...
	public static void SaveHighScore (int newHighScore) {
		if (newHighScore < HighScore || HighScore == 0) { 
			...
			PlayerPrefs.SetInt ("sumHS", newHighScore); // Store high score in player prefs
			if (MgrSet ())
				mgr.UpdatedHS (); // Notify manager of change
		}
	}
